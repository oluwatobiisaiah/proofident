...
function buildTundeBets() {
  return Array.from({ length: 36 }, (_, index) => ({
    ...
    betAmount: 3500 + (index % 5) * 500,
    odds: (1.7 + (index % 3) * 0.2).toFixed(2),
    outcome: index % 4 === 0 ? "loss" : "win",
    payoutAmount: index % 4 === 0 ? null : 7000 + (index % 3) * 1000,
...
function buildTundeTransactions() {
  return Array.from({ length: 48 }, (_, index) => ({
    ...
    amount: index % 6 === 0 ? 22000 : 3500 + (index % 5) * 2000,
    balanceAfter: 50000 - index * 500 + (index % 6 === 0 ? 15000 : -1000),
...
function buildChiomaTransactions() {
  return Array.from({ length: 60 }, (_, index) => ({
    ...
    amount: index % 4 === 0 ? 18000 + (index % 3) * 4000 : 4000 + (index % 6) * 1500,
    balanceAfter: 65000 - index * 300 + (index % 4 === 0 ? 12000 : -800),
...
function buildAminaTransactions() {
  return Array.from({ length: 8 }, (_, index) => ({
    ...
    amount: index % 3 === 0 ? 7000 : 2500 + index * 250,
    balanceAfter: 12000 - index * 600,
    ...
    await db.insert(jobs).values([
      {
        ...
        startupCosts: { bikeDeposit: 85000, safetyGear: 0 },
      ...
      },
      {
        ...
        startupCosts: { transportFloat: 25000, smartphoneTopup: 15000 },
        minimumIncome: 90000,
        maximumIncome: 140000,
